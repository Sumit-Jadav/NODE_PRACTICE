...
const localstorage = new LocalStorage("./file-creation-cli-app");

let counter = parseInt(localstorage.getItem("num")) || 0;
      ...
      rl.question("Enter data :- ", (data) => {
        fs.writeFile(dDrivePath, data, (err) => {
          if (err) console.log("Error in file creation :- ", err);
            ...
            let cou = parseInt(localstorage.getItem("num"), 10);
            cou = cou + 1;
            localstorage.setItem("num", cou);
          }
          showMenu();
        });
      });
